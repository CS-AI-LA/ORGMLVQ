...
num_classes = length(unique(this.trainingData.labels));
...
for i = 1:this.trainingData.nFeatureVectors
    ...
    jwin = correct(JJ);   % J
    kwin = incorrect(KK); % K
    wJ = prototypes(jwin, :); % WJ
    wK = prototypes(kwin, :); % Wk
    
    % GMLVQ prototype update for one example fvi
    DJ = (currentFV - wJ)'; % displacement vector
    DK = (currentFV - wK)';
    normalizationFactor = (dJ + dK)^2;
    dwJ = -(dK/normalizationFactor)*lambda*DJ; % mu+ . lambda . (x - WJ) 
    dwK = (dJ/normalizationFactor)*lambda*DK;  % mu- . lambda . (x - WK) 
    
    %% ORGMLVQ: Updates for R
    lamb = getGlobalx;
    [dCorrect,dIncorrect] = get_derivative_R(prototypes,num_classes,jwin,kwin);
    dwJ = dwJ + (lamb * dCorrect');
    dwK = dwK + (lamb * dIncorrect');
